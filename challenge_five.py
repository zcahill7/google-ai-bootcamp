...
Original file is located at
    https://colab.research.google.com/embedded/projects/qwiklabs-gcp-03-d3c5d190e927/locations/us-central1/repositories/0e52af91-68b9-496f-8128-b41516d60eff
...
Step 1: Create dataset for challenge five then import the data into a table for this challenge
"""
...
# CREATE SCHEMA IF NOT EXISTS challenge_five
# OPTIONS (
#   location="us",
...
!bq load --autodetect --source_format=CSV --skip_leading_rows=1 --field_delimiter=',' challenge_five.orig_data gs://labs.roitraining.com/data-to-ai-workshop/airports.csv

# Commented out IPython magic to ensure Python compatibility.
# # Show table from BigQuery
# # Store results in get_orig_data
# %%bigquery get_orig_data
# SELECT * FROM challenge_five.orig_data

# Show results from the load table
get_orig_data

"""Step 2: Define a python function to use an API call to pull weather forcast data from the NWS API"""

# Function that returns the weather forecast from the NWS using Lat/Long point
...
"""Step 3: Create a default connection to the Gemini model to be used for creating a weather forcast for each large airport"""

# Commented out IPython magic to ensure Python compatibility.
# # Add Gemini model within the challenge_five dataset using default connection
# # Store results in create_model_results
# 
# %%bigquery create_model_results
# CREATE OR REPLACE MODEL
#   `challenge_five.gemini_model` REMOTE
# WITH CONNECTION DEFAULT OPTIONS (ENDPOINT = 'gemini-2.0-flash');

"""Step 4: Get large airports in the US from the orig_data table and write to new table"""

# Commented out IPython magic to ensure Python compatibility.
# # Running this code will get a table of large airports in the United States
# # Store results in get_large_US_airports
# 
# %%bigquery get_large_US_airports
# CREATE OR REPLACE TABLE challenge_five.just_large_airports AS (
#   SELECT * FROM challenge_five.orig_data
#   WHERE iso_country = "US" AND type = "large_airport"
# );

# Commented out IPython magic to ensure Python compatibility.
# # Show results of SQL query to get large airports in the US
# 
# %%bigquery get_just_large_airports_table
# SELECT * FROM challenge_five.just_large_airports

# Show the get_just_large_airports_table
get_just_large_airports_table

"""Step 5: Get the just_large_airports_data and get the weather data for these airports"""
...
print("✅ Initialization Complete: Vertex AI and BigQuery clients are ready.")
...
print("\n🚀 Starting main processing logic...")
...
print(f"✅ Successfully created and populated table '{destination_table_id}'.")
